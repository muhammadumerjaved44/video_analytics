...
async def upload_frames(profile_name, frame_no):
    bucket_name = "frames"
    minio_client = Minio(host, access_key=access_key, secret_key=secret_key, secure=False)
    found = minio_client.bucket_exists(bucket_name)
    if not found:
    ...
    try:
        image_path = os.path.realpath(f"./download_images/all_images/image_{frame_no}.jpg")
        print(image_path)
        ...
        frame_name_path = os.path.join(profile_name, f"image_{frame_no}.jpg")
...
async def get_videos_as_object():
    ...
    try:
        response = minio_client.presigned_get_object(
            ...
            version_id="dda4f765-7227-4997-b1a0-805996e6300c",
...
async def get_unprocessed_videos_urls(video_id):
