...
from models import (fetch_image_from_url, insert_object,
                    update_qr_frame_flags)
...
async def qr_to_text(main_file_url, frame_id, video_id):
    ...
    if barcodes:
        final_string = codecs.decode(barcodes[0].data.decode(), 'unicode_escape').replace('\r', ' ').strip()
    else:
        final_string = ''
